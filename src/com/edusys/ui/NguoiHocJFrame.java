    ...
    void fillTable(){
        ...
        try {
            ...
            for (NguoiHoc nh : list) {
                Object[] row = {nh.getMaNH(), nh.getHoTen(), nh.getGioiTinh() ? "Nam" : "Nữ", DateHelper.toString(nh.getNgaySinh(),"dd/MM/yyyy"), nh.getDienThoai(), nh.getEmail(), nh.getMaNV(), DateHelper.toString(nh.getNgayDK(),"dd/MM/yyyy")};
    ...
    void updateStatus(){
     ...
     public boolean validateForm(){
        ...
        if(txtMaNH.getText().length() == 0){
            ...
            MsgBox.alert(this, "Mã sinh viên phải có PS và 5 ký tự số ! VD : PS15011 ");
            txtMaNH.requestFocus(); 
            return false;
            ...
            MsgBox.alert(this, "Vui lòng nhập đúng định dạng số VN !");
            txtDienThoai.requestFocus();
            return false;
        ...
        List<NguoiHoc> list = dao.selectAll();
            for (NguoiHoc nh : list) {
                if (txtMaNH.getText().equals(nh.getMaNH())) {
                    MsgBox.alert(this, "Mã người học đã tồn tại ! Vui lòng nhập mã khác !");
                    txtMaNH.requestFocus();
                    return;
                }
            } 
