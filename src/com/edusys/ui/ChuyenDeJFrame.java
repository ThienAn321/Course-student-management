    ...
    void updateStatus(){
     ...
     public boolean validateForm() {
        if(txtMaCD.getText().length() == 0) {
            ...
            MsgBox.alert(this, "Mã chuyên đề không được quá 5 ký tự ! !");
        ...
        try {
            ...
            MsgBox.alert(this, "Không được phép nhập chữ ở ô thời lượng !");
            txtThoiLuong.requestFocus();
            return false;
        }
        ...
        try {
            if(Double.parseDouble(txtHocPhi.getText()) <= 0) {
                ...
                MsgBox.alert(this, "Không được phép nhập chữ ở ô học phí !");
                txtHocPhi.requestFocus();
                return false;
        }
...
//            }         
    ...
    private void btnInsertActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnInsertActionPerformed
        List<ChuyenDe> list = dao.selectAll();
            for (ChuyenDe cd : list) {
                if (txtMaCD.getText().equals(cd.getMaCD())) {
                    MsgBox.alert(this, "Mã Chuyên đề đã tồn tại ! Vui lòng nhập mã khác !");
                    txtMaCD.requestFocus();
                    return;
                }
            }         
